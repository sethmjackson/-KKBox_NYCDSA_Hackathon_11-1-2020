...
def roundTraditional(number, ndigits = 2):
...
def getTimeDiff(timeDiff: float, printTime = False):
    ...
    timeString = ''
    timeList = {'Days: ': days, 'Hours: ': hours, 'Minutes: ': minutes}
    for name, time in timeList.items():
        if time > 0:
            timeString += name + str(time)+ '  '

    if seconds < 1:
        seconds = round(seconds , 2)
    else:
        seconds = int(seconds)
    timeString += 'Seconds: '+ str(seconds)
